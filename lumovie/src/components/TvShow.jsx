    ...
    const fetchTvShows = async () => {
      try {
        const response = await axios.get(
          ` https://phimapi.com/v1/api/danh-sach/phim-bo?page=1&sort_field=_id&sort_type=asc&sort_lang=&category=&country=&year=&limit=10`
        );
    ...
    <section className="w-full flex items-start justify-center flex-col gap-5 font-manrope">
      ...
      <div className="w-full h-[500px] relative ">
        ...
        <Swiper
          ...
          className="tvShowSwiper w-full h-full "
          loop={true}
          spaceBetween={25}
          breakpoints={{
            320: {
        ...
        >
          {tvShowData !== undefined &&
            tvShowData.map((tvShow, index) => (
              <SwiperSlide
                className="tv_show_item"
                key={index}
                onClick={() => {
                  handleNavigate(tvShow.slug);
                }}
              >
                <div
